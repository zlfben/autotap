...
export class ParameterConfigurationComponent {
  ...
  constructor(public userDataService: UserDataService, private route: ActivatedRoute, 
  ...
  selectHistoryClauseGate(parameterKey: number, temporality: string) {
    ...
    this.userDataService.selectSpHistoryClause(this.userDataService.currentSpType, parameterKey, this.parameterVals, temporality);
  }
  ...
  completeParameterSelect() {
    ...
    if (valid == false) {
      ...
      if (this.userDataService.historyClause) {
        ...
        this.userDataService.reloadForSpHistoryClause();
      } else {
        if (this.userDataService.currentlyStagedSp1) {
          this.userDataService.addClauseToSp1();
          this.userDataService.gotoCreateSp(1);
        }
        else if (this.userDataService.currentlyStagedSp2) {
          this.userDataService.addClauseToSp2();
          this.userDataService.gotoCreateSp(2);
        }
        else {
          this.userDataService.addClauseToSp3();
          this.userDataService.gotoCreateSp(3);
        }
      }
    }
  }
